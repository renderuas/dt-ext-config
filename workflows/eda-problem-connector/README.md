...
- **`customFilter`: `(maintenance.is_under_maintenance == false or isNull(maintenance.is_under_maintenance))`** — `davis-problem` ("Problem Trigger") **no tiene en la UI** un control equivalente a `maintenanceWindowTriggerBehavior: outside` de `eda-event-connector` (confirmado por el usuario directamente en el tenant, y consistente con que el struct `DavisProblemConfig` del Terraform provider no lista ese campo). La exclusión de mantenimiento se resuelve aquí con una expresión en el propio `customFilter`, que sigue disponible en `davis-problem`.
  - El campo `customFilter` de un trigger **no acepta DQL completo**, acepta un lenguaje "matcher" más restringido (el mismo que usa OpenPipeline/Log processing — ver [DQL matcher en OpenPipeline](https://docs.dynatrace.com/docs/discover-dynatrace/platform/openpipeline/reference/dql-matcher-in-openpipeline)). Soporta `==`, `<`, `>`, `and`/`or`/`not`, `matchesValue()`, `matchesPhrase()`, `isNull()`, `isNotNull()` — **pero no `isFalseOrNull()`/`isTrueOrNull()`**, que son funciones del pipeline DQL completo (`fetch | filter`) y no existen en el matcher. Una primera versión de este filtro usaba `isFalseOrNull(...)` y **Dynatrace rechazó la importación del workflow** con un error de parseo (`error.json` en este directorio) — corregido a la forma equivalente con `== false or isNull(...)`, que sí está documentada como soportada por el matcher.
  - Se evita `maintenance.is_under_maintenance == false` a secas porque las comparaciones de igualdad en DQL/matcher usan lógica de tres estados (`true`/`false`/`null`): si el campo no existe en el registro, `campo == false` se evalúa como `null` (no como `true`), y el problema quedaría excluido por error. El `or isNull(...)` cubre el caso de campo ausente — misma semántica de "no se sabe que esté en mantenimiento, dejar pasar" que `eda-event-connector` resolvía con su doble condición OR en DQL ad-hoc.
...
| `error.json` | Error de importación recibido al usar `isFalseOrNull()` en el `customFilter` del trigger (función no soportada por el matcher) — referencia de por qué el `customFilter` actual usa `== false or isNull(...)` |
...
5. **Ajustar las `categories` activas** según el volumen real observado con "Query past events" en la UI o con `trigger-problems.dql`
...
- [x] **Excluir mantenimiento sin control nativo en la UI de "Problem Trigger"** — confirmado por el usuario que no existe ese control (a diferencia de `davis-event`); resuelto vía `customFilter`
- [x] **Corregir `customFilter` rechazado al importar** — la primera versión (`isFalseOrNull(...)`) hizo fallar la importación del workflow (`error.json`: "Caught exception when parsing DQL query..."), porque el `customFilter` del trigger usa el lenguaje "matcher" restringido de Dynatrace, no DQL completo, y ese matcher no tiene `isFalseOrNull()`. Corregido a `maintenance.is_under_maintenance == false or isNull(maintenance.is_under_maintenance)`, confirmado contra la [documentación del DQL matcher](https://docs.dynatrace.com/docs/discover-dynatrace/platform/openpipeline/reference/dql-matcher-in-openpipeline)
